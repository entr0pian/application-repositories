...
| `environments/<env>/*.yaml` | `git` **files** generator | one Application per file, named `<file>-<env>` (e.g. `payments-db-management`) | `Release`/`Database` CRs are per component *and* per environment; each CR file syncs on its own into a namespace named after its folder |

Neither reads a paired `values/` file — every field a `Release` or `Database` CR needs is inline in its own manifest, so there's no identity/values split to make (same reasoning as `packages/`, different shape). `registry/*.yaml` Applications each scope their `directory.include` to just that one file (so ten components' CRs don't get bundled into one Application); `environments/<env>/*.yaml` works the same way, one Application per CR file, so each `Release`/`Database` has its own sync and health status — dropping in a new `checkout-db.yaml` next to `checkout-release.yaml` needs no ApplicationSet change, same philosophy as `infra/`/`packages/`. Don't rely on these names to find an Application: every Application from `taskapp-catalog` and `taskapp-platform` carries the same `platform.taskapp.io/{component,environment,type,name}` labels (`type` is `service`, `component`, `database` or `release`; `name` is what it delivers), and Backstage finds Applications by those labels only, e.g. the Database page's "Open in Argo CD" link queries `type=database,environment=<env>,name=<cr>`.
